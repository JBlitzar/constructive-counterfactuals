...
class TinyImageNetDataset(Dataset):
    def __init__(self, root, transform=None, split="train"):
        ...
        self.all_files = glob.glob(
            os.path.join(os.path.join(root, split), "*/images/*.JPEG")
        )
        ...
        with open(os.path.join(root, "wnids.txt"), "r") as f:
            ...
            with open(os.path.join(root, "clsloc.txt"), "r") as clsloc:
                ...
                for idx, id in enumerate(wnids):
                    try:
                        self.wnids[id] = [
                            torch.tensor(int(clsloc_dict[id][0])).type(torch.IntTensor),
                            clsloc_dict[id][1],
                        ]
                    except (KeyError, IndexError):
                        self.wnids[id] = [
                            torch.tensor(0).type(torch.IntTensor),
                            "NOT_FOUND",
                        ]
